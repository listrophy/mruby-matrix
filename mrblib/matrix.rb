...
class Matrix
  ...
  class ErrDimensionMismatch < StandardError; end
  class ErrNotRegular < StandardError; end
  class ErrOperationNotDefined < StandardError; end
  class ErrOperationNotImplemented < StandardError; end
  ...
  def Matrix.empty(row_size = 0, column_size = 0)
    raise ArgumentError.new "One size must be 0" if column_size != 0 && row_size != 0
    raise ArgumentError.new "Negative size" if column_size < 0 || row_size < 0
  ...
  def minor(*param)
    ...
    when 4
      ...
      raise ArgumentError.new param.inspect
    end
  ...
  def *(m) # m is matrix or vector or number
    ...
    when Matrix
      ...
      return apply_through_coercion(m, :*)
    end
  end
  ...
  def +(m)
    ...
    raise ErrDimensionMismatc unless row_size == m.row_size and column_size == m.column_size
  ...
  def -(m)
    ...
    raise ErrDimensionMismatch unless row_size == m.row_size and column_size == m.column_size
  ...
  def /(other)
    ...
    when Matrix
      ...
      return apply_through_coercion(other, :/)
    end
  end
  ...
  def ** (other)
    ...
    when Integer
      ...
      z = nil
      loop do
        z = z ? z * x : x if other % 2 == 1
        return z if (other >>= 1) == 0
        x *= x
      end
    when Numeric
  ...
  #
  def transpose
    return Matrix.empty(column_size, 0) if row_size == 0
    next_matrix = (0...column_size).inject([]) do |memo, i|
      memo + [@rows.map{|r| r[i]}]
    end
    new_matrix next_matrix, row_size
  end
  alias t transpose
  ...
  def coerce(other)
    ...
    when Numeric
      ...
      raise TypeError.new "#{self.class} can't be coerced into #{other.class}"
    end
  end
    ...
    def convert_to_array(obj, copy = false) # :nodoc:
      ...
      when Vector
        ...
        begin
          ...
          raise TypeError.new "can't convert #{obj.class} into an Array (#{e.message})"
        end
        raise TypeError.new "#{obj.class}#to_ary should return an Array" unless converted.is_a? Array
        converted
      end
    end
  ...
  module CoercionHelper # :nodoc:
    ...
    def apply_through_coercion(obj, oper)
      ...
      raise TypeError.new "#{obj.inspect} can't be coerced into #{self.class}"
    end
    ...
    def self.coerce_to(obj, cls, meth) # :nodoc:
      ...
      begin
        ...
        raise TypeError.new "Coercion error: #{obj.inspect}.#{meth} => #{cls} failed:\n(#{e.message})"
      end
      raise TypeError.new "Coercion error: obj.#{meth} did NOT return a #{cls} (was #{ret.class})" unless ret.kind_of? cls
      ret
    end
...
class Vector
  ...
  def each2(v) # :yield: e1, e2
    raise TypeError.new "Integer is not like Vector" if v.kind_of?(Integer)
  ...
  def *(x)
    ...
    when Vector
      raise Matrix::ErrOperationNotDefined
    else
      apply_through_coercion(x, :*)
    end
  end
